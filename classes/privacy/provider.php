...
/**
 * Privacy Subsystem implementation for block_createusers.
 *
...
namespace block_createusers\privacy;
...
class provider implements
        // The block_createusers block stores user provided data.
        \core_privacy\local\metadata\provider,

        // This plugin is capable of determining which users have data within it.
        \core_privacy\local\request\core_userlist_provider,

        // The block_createusers block provides data directly to core.
        \core_privacy\local\request\plugin\provider {

    /**
     * Returns information about how block_createusers stores its data.
     *
    ...
    public static function get_users_in_context(userlist $userlist) {
        ...
        $sql = "SELECT bpc.instanceid AS userid
                 ...
                 WHERE bi.blockname = 'createusers'
                   AND bpc.contextlevel = :contextuser
    ...
    public static function export_user_data(approved_contextlist $contextlist) {
        ...
        foreach ($instances as $instance) {
            ...
            $block = block_instance('createusers', $instance);
            if (empty($block->config)) {
                // Skip this block. It has not been configured.
                continue;
            }

            $createusers = writer::with_context($context)
                ->rewrite_pluginfile_urls([], 'block_createusers', 'content', null, $block->config->text);

            // Default to FORMAT_createusers which is what will have been used before the
            // editor was properly implemented for the block.
            $format = isset($block->config->format) ? $block->config->format : FORMAT_createusers;

            $filteropt = (object) [
                'overflowdiv' => true,
                'noclean' => true,
            ];
            $createusers = format_text($createusers, $format, $filteropt);

            $data = helper::get_context_data($context, $user);
            helper::export_context_files($context, $user);
            $data->title = $block->config->title;
            $data->content = $createusers;
    ...
    public static function delete_data_for_all_users_in_context(\context $context) {
        ...
        // The only way to delete data for the createusers block is to delete the block instance itself.
    ...
    protected static function get_instance_from_context(\context_block $context) {
        ...
        return $DB->get_record('block_instances', ['id' => $context->instanceid, 'blockname' => 'createusers']);
    }
}
